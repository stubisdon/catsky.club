...
- Logged-in view shows:
  ...
  - account + free-member upgrade links use direct Ghost-managed anchors (`href="#/portal/..."` + `data-portal="..."`) so Portal handles real user clicks without synthetic trigger indirection.
...
- synthesizes a narrowly-scoped fallback for empty JSON member-session reads (`{ member: null }` for `/members/api/member/`) while preserving native response bodies for newsletter mutation endpoints (`/members/api/member/newsletters/...`),
- limits JSON payload rewriting to Ghost content settings/tiers reads and member-session reads so member mutation payloads are passed through unchanged,
...
- Tracks can define `announcedReleaseDate` to mark finished songs that free registered users can access, and optionally `availableFrom` + `lockedLabel` metadata for explicit UTC date-based embargoes.
...
- accessible vs locked tracks computed client-side from tier, announced release-date eligibility for free members, plus any explicit track-level embargo date.
