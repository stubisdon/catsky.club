...
Express now intercepts tokenized `GET /unsubscribe?uuid=...&key=...&newsletter=...` and `GET /unsubscribe/?uuid=...&key=...&newsletter=...` links before SPA fallback, performs the actual unsubscribe by sending a `POST` to Ghost’s `/unsubscribe/` controller on `GHOST_INTERNAL_URL` (default `http://127.0.0.1:2368`), and then returns a Catsky-hosted confirmation page so users receive explicit unsubscribe feedback. This `POST` detail matters: upstream Ghost treats `GET /unsubscribe/...` as a redirect/confirmation entry point and only executes the unsubscribe on `POST`, so a GET-only proxy can look successful without changing newsletter preferences. Those Express-to-Ghost requests intentionally send the canonical public `Host`, `X-Forwarded-Host`, `X-Forwarded-Proto`, and `X-Forwarded-Port` values derived from `GHOST_URL`, because Ghost newsletter unsubscribe handling can fail when the upstream request arrives as bare localhost/internal origin traffic. Non-tokenized `/unsubscribe` routes still proxy through to Ghost, and upstream redirects from internal/localhost hosts are rewritten to the public request origin (or `GHOST_URL` fallback). Express also includes defensive pass-through handlers for `/content/images/*` and `/r/*` so Ghost favicon/email paths still resolve if nginx route blocks are accidentally stale; those fallback fetches forward the same canonical public host/proto/port contract so Ghost does not bounce them back to the same public URL in a canonical-host redirect loop. nginx remains the primary owner for these prefixes in production.
...
- E2E: Playwright suite in `e2e/` with smoke-vs-matrix commands:
  ...
  - The Chromium Playwright projects opt into Playwright's `channel: 'chromium'` launch path so smoke coverage runs against the full Chromium binary/new headless mode instead of the separate headless-shell package.
