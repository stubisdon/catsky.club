...
- `/welcome` should remain visible while session hydration completes; if delayed, it must keep the form usable, leave the primary continue CTA clickable once first name is entered, attempt an inline session recheck on submit, and offer a retry action without ejecting the user back to `/connect` or presenting the delay as a hard form error.
